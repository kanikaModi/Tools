...
public class RMF_RadialMenu : MonoBehaviour {
	...
	public RMF_RadialMenuElement prefab_element;
	public ToolParameters[] toolpara;
	//public int elementCount = 2;
	//private List<RMF_RadialMenuElement> elements = new List<RMF_RadialMenuElement>();
   ...
   private int elementCount;
    ...
    void Awake() {
        ...
        for (int i = 0; i < elementCount; i++) {
			elements [i] = (RMF_RadialMenuElement) Instantiate (prefab_element, this.transform);
			elements [i].GetComponent<RMF_RadialMenuElement> ().Product_name = toolpara [i].product_name;
			elements [i].GetComponent<RMF_RadialMenuElement> ().label = toolpara [i].series_name;
			elements [i].GetComponent<RMF_RadialMenuElement> ().sprite = toolpara [i].sprite_name;
            if (elements[i] == null) {
